...
from db.pydantic_schema.game import GameCreate
from db.pydantic_schema.player import PlayerResponse
from db.db_manager import get_db_session
from db.models import Player, Team, User, Game, GameInRoster
from utils import get_current_user_id, get_current_user_and_team, ensure_game_exists, ensure_team_owns_game
...
def get_game(game_id: int, db_session: Session = Depends(get_db_session), user_and_team: tuple[User, Team] = Depends(get_current_user_and_team)):
...
def delete_game(game_id: int, already_confirmed: bool, db_session: Session = Depends(get_db_session), user_and_team: tuple[User, Team] = Depends(get_current_user_and_team)):
    _, team = user_and_team
    game = ensure_game_exists(game_id, db_session)
    ensure_team_owns_game(game, team)
...
def get_games_for_user(db_session: Session = Depends(get_db_session), user_and_team: tuple[User, Team] = Depends(get_current_user_and_team)):
    try:
        _, team = user_and_team
        games = db_session.query(Game).filter(Game.team == team).all()
        return games
    except Exception as e:
...
async def scrape_roster(game_url: str, home: Literal["home", "away"], db_session: Session = Depends(get_db_session), user_and_team: tuple[User, Team] = Depends(get_current_user_and_team)):
    roster = await scrape_team_slots(game_url, home)
    _, team = user_and_team
    players_in_team = db_session.query(Player).filter(Player.team == team).all()
