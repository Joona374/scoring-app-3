...
export default function ChemistrySection({ chemistryData }) {
        ...
        <ResponsiveContainer width="100%" height={400}>
          <ScatterChart margin={{ top: 20, right: 30, bottom: 30, left: 10 }}>
            ...
            <XAxis 
              ...
              fontSize={11}
              domain={[0, maxX || 1]}
              tickCount={maxX > 2 ? 6 : 3}
              label={{ value: 'Yhteiset MP / peli', position: 'bottom', fill: 'var(--muted-text)', fontSize: 12, offset: 10 }}
            />
            <YAxis 
              type="number" 
              dataKey="y" 
              ...
              fontSize={11}
              domain={[0, 50]}
              label={{ value: 'Maalintekotehokkuus %', angle: -90, position: 'insideLeft', fill: 'var(--muted-text)', fontSize: 12, offset: -5 }}
            />
            ...
            <ReferenceLine 
              ...
              strokeDasharray="3 3" 
              label={{ value: 'Joukkueen KA', position: 'top', fill: 'var(--muted-text)', fontSize: 10 }}
            />
            <ReferenceLine 
              y={avgEff} 
              stroke="rgba(255,255,255,0.4)" 
              strokeDasharray="3 3" 
              label={{ value: 'KA Tehokkuus', position: 'right', fill: 'var(--muted-text)', fontSize: 10 }}
            />
  ...
  return (
    <section className="chemistry-section">
      <div className="section-header-row unified-row">
        <h3 className="section-title-small">Yhteistyö ja Kemia</h3>
        
        <div className="header-tabs-container">
          <div className="filter-tabs pos-filters">
            <button 
              className={`filter-tab ${posFilter === "ALL" ? "active" : ""}`}
              onClick={() => setPosFilter("ALL")}
            >
              Kaikki
            </button>
            <button 
              className={`filter-tab ${posFilter === "FORWARD" ? "active" : ""}`}
              onClick={() => setPosFilter("FORWARD")}
            >
              Hyökkääjät
            </button>
            <button 
              className={`filter-tab ${posFilter === "DEFENDER" ? "active" : ""}`}
              onClick={() => setPosFilter("DEFENDER")}
            >
              Puolustajat
            </button>
          </div>
